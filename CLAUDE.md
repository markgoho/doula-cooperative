...
export const contactUsForm = onRequest((request, response) =>
  import("./contact-us-form/handler.js").then((m) => m.handler(request, response))
);
```
...
- **Hugo:** `hugo/CLAUDE.md` - Template system, content structure, SCSS architecture
- **Functions:** `functions/CLAUDE.md` - Function types, external integrations, secret management
- **Members:** `members/CLAUDE.md` - Angular patterns, routing, services, testing guidelines
