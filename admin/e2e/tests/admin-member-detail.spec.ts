...
test.describe('Admin Member Detail Page', () => {
  ...
  test('admin performs clean slate delete on a member', async ({ authenticatedAdminPage }) => {
    ...
    await authenticatedAdminPage.waitForURL('**/members');
    await expect(
  ...
  test.skip('admin views member profile content', async ({ authenticatedAdminPage }) => {
    ...
    await expect(editProfileLink).toHaveAttribute(
      'href',
      '/members/test-member-123/profile/edit',
    );
    ...
    const previewPageUrl = '/members/test-member-123/profile';
    await authenticatedAdminPage.goto(previewPageUrl);
    await authenticatedAdminPage.waitForURL('**/members/test-member-123/profile');

    // === Navigate to edit page from detail page ===
    await editProfileLink.click();
    await authenticatedAdminPage.waitForURL('**/members/test-member-123/profile/edit');
    await expect(
    ...
    await authenticatedAdminPage.waitForURL('**/members/test-member-123');
