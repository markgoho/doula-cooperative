...
import { handleApi, type Logger } from "../../src/api/handler.js";
import type { Request } from "firebase-functions/v2/https";
import type { Response } from "express";

/**
 * Tests for the API handler (entry point).
 *
 ...
 *   bun test test/api/handler.test.ts
 */
describe("handleApi", () => {
  describe("Error handling", () => {
    it("should handle errors and log them", async () => {
      ...
      await handleApi(badRequest, mockResponse, mockLogger);

      // Verify error was logged
      expect(errorMock).toHaveBeenCalledTimes(1);
      expect(Array.isArray(errorMock.mock.calls[0])).toBe(true);
      expect(errorMock.mock.calls[0]?.[0]).toBe("Elysia API handler failed");
    ...
    it("should handle successful requests without errors", async () => {
      ...
      await handleApi(mockRequest, mockResponse, mockLogger);
